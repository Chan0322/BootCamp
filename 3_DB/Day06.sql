...
-- 시퀀스 CYCLE/CACHE
-- CYCLE : 시퀀스 값이 최대값OR최소값 도달했을 때, 반대의 값부터 시작하는 옵션
...
SELECT SEQ_CYCLE.NEXTVAL FROM DUAL; --230
...
-- CACHE / NOCACHE
-- 컴퓨터가 다음 값에 대한 연산들을 미리 계산해 놓는 것.
...
-- 오라클에서 자동으로 생성하는 인덱스.
-- 해당 컬럼을 SELECT 문으로 조회할 때 포함 시키면
...
CREATE OR REPLACE PROCEDURE DEL_ALL_TMP
IS
	-- 변수선언...
BEGIN
	DELETE FROM EMP_TMP;
...
INSERT INTO MYBOARD
VALUES(SEQ_MYBOARD.NEXTVAL, '이창진', '3교시', '배가 고프네... 밥먹으러 갑시다.', SYSDATE);